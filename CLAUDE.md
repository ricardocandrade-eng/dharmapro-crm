...
|---|---|---|
| 02/07/2026 16:53 | (pendente — aguardando push) | **feat(verohub): captura de pedido do VeroHub → venda no CRM sem copiar/colar.** Direção nova (inversa ao botão "Criar Pedido" de 30/06, que é CRM→VeroHub): a extensão `extensao-dharmapro` (v2.7.0→**2.8.0**) passa a injetar um botão flutuante **"Enviar pro CRM"** na página de um pedido (`https://hub.veronet.com.br/sales/{id}`). O pedido inteiro já vem no objeto JS `window.__SALE` da página — não precisa de scraping de DOM. **Extensão** (3 arquivos novos + 2 editados): `verohub-main-world.js` (MAIN world — lê `window.__SALE` por whitelist explícita de 40 campos; `auth_token`, selfie, documentos, 2FA, serasa, liveness NUNCA saem do MAIN world) + `content-verohub.js` (isolated — botão + painel de conferência; "os dois" caminhos: **Revisar** abre painel com os dados antes de gravar, **enviar direto** manda na hora) + `background.js` (handler `verohub.enviar` → POST `text/plain` no `doPost`, evitando preflight; URL/secret via `chrome.storage.local`: `verohub_crm_url`/`verohub_crm_secret`) + `manifest.json` (host_permissions `hub.veronet.com.br`/`script.google.com`/`googleusercontent.com` + os 2 content scripts). **Backend**: novo `VerohubAPI.js#_verohubCapturarVenda_` + 1 bloco de rota no `doPost` (`action:'verohub_capture'`, secret próprio `VEROHUB_CAPTURE_SECRET` com fallback pro `webhook_secret`). Normalização 100% server-side reusando a trilha do CRM: plano resolvido **pelo código VeroHub** (`sale.plan`→nome via `_getVerohubCodigos().codigos[cod].nome`, forward/determinístico; código vai direto pra `COD_PLANO`, pontos saem dele), cidade/UF por `buscarCEPBackend(zip_code)`, rua/nº/bairro do próprio pedido, gravação via `_construirLinhaDados`, e **Móvel do combo** criado via `criarVendaMovelVinculada` + `_inferirMovelComboFromFibra_` quando há `mvno_plan`. Idempotente (dedupe pela coluna `VEROHUB_PEDIDO`). Pedido nasce em `1- Conferencia/Ativação`, canal `VEROHUB`, nº do pedido em `VEROHUB_PEDIDO`, e-mail/proposta/status VeroHub na Observação. **Decisão**: "Revisar" ficou como painel na própria página do VeroHub (não pré-fill da Nova Venda) — sem risco de mexer no wizard. **Validação**: `node --check` nos 3 arquivos novos OK; dry-run do pipeline contra pedido real #2067020 (42 campos, **zero vazamento sensível**, Fibra Combo íntegra: cliente/CPF/endereço/cod 5676/R$139,90/venc 13/móvel DDD32). **Setup pós-deploy (Ricardo)**: (1) Script Property `VEROHUB_CAPTURE_SECRET`; (2) `chrome.storage.local.set({verohub_crm_secret:'<mesmo valor>'})` no service worker + recarregar extensão v2.8.0 em cada máquina BKO. **Fix combo (02/07 pós-teste):** o nome do plano deixou de depender do dicionário `verohub_codigos_cidades.json` (sweep 21/05, defasado — não tinha os códigos NP 3.0, ex. 5676): `verohub-main-world.js` agora lê o nome real na API da própria página (`/api/plans_svas` → `plans[].name`) e o backend prioriza `sale.plan_name` (fallback dicionário). Além disso, a inferência do Móvel passou a extrair o GB tanto de "MÓVEL XXGB" quanto de "MAIS CONECTADO XXGB" (nomenclatura VeroHub), montando um nome sintético pro `_inferirMovelComboFromFibra_`. Sem isso, combos gravavam PLANO em branco e não criavam o Móvel. **Não testado com insert real** (depende do deploy + reload da extensão). Arquivos: `VerohubAPI.js`, `Code.js` (doPost), `extensao-dharmapro/{content-verohub.js,verohub-main-world.js,background.js,manifest.json}`, `Config.js` (DEPLOY_DATE). |
| 02/07/2026 | (via Action — commit `5f1f359` + 5 anteriores) | **feat(pap): pacote de ajustes no portal do parceiro (`pap.mobilefibra.com.br` = `Parceiros.html`) + rename no CRM.** (1) **Menu inicial** reordenado: Minhas Vendas → Máscara de Venda (ex "Pré-Venda") → Pontos & Prêmios (desabilitado, "Em breve") → **Mapa de Ofertas** (card novo, mobile-first, busca cidade via datalist → `getOfertasCidade`). (2) **Minhas Vendas**: abre em "Vendas Ativas" com filtro "Todos"; filtro "Aguardando"→"Ag. Instalação"; badges do topo viram 3 (Instaladas / Ag. Instalação / Vendas Ativas) escopados por **chips de mês nomeado** (corrente + 2 anteriores) + valores **A receber / Recebido**; card mostra "Status Pagamento". (3) **Máscara de Venda**: tela renomeada (tag "Máscara" / "Efetuar Pedido"); botão **"Buscar dados do cliente"** (Assertiva) preenche Nome + novos campos Data de Nascimento e Nome da Mãe — `consultarAssertivaGAS` passa a retornar nascimento/nomeMae, `salvarPreVenda` persiste (cols 27/28 da aba `Pré-Vendas`) e `aprovarPreVenda` leva pra venda (cols Y/Z `NOME_MAE`/`DT_NASC` via `_construirLinhaDados`); dropdown de plano passa a vir de `getOfertasCidade` (mesma fonte do Mapa de Ofertas, com filtro `PUBLICAR`), com **fallback** para os planos do `buscarCEPBackend`. (4) **Controle de custo da Assertiva** (todas as consultas passam por `consultarAssertivaGAS`): valida dígito verificador do CPF, **dedupe/cache 30 dias** por CPF (reusa sem cobrar), **limite 5 consultas PAGAS/dia por parceiro**, e log de auditoria na aba nova `Log Assertiva PAP` (origem PAGO/CACHE/BLOQUEADO/INVALIDO). Constantes `PAP_ASSERTIVA_LIMITE_DIA=5` / `PAP_ASSERTIVA_CACHE_DIAS=30`. (5) **Backend `getMeusPagamentosPAP`** separa `STATUS_PAP` "Em Aberto" (a receber) de "Pago" (recebido) — retorna `totalPago`/`qtdPago`/`itensPagos` (aditivo). (6) **CRM**: item de menu lateral, breadcrumb, título da página (`FilaPAP.html`) e label de permissões "Fila PAP" → **"Máscara de Vendas"** (ids internos `filaPAP`/`menuFilaPAP` intactos). Arquivos: `Parceiros.html`, `ParceirosAPI.js`, `Index.html`, `JS.html`, `Usuarios.html`, `FilaPAP.html`. Validação: `node --check` nos backends + `<script>` dos HTML; validador de CPF testado (6 casos); camada de planos sadia em prod (359 cidades, 32 planos JF). Deploy confirmado (`DEPLOY_DATE` servido 02/07). |
...
| 05/06/2026 18:18 | deploy manual @880 | **ux(dashboard): barra de instaladas passa a decompor instalacoes do mes entre venda do mes e backlog anterior.** `Code.js#getDashboard` agora retorna `instalacoesVendaDoMes` e `instalacoesVendasAnterioresMes`; `Dashboard.html` renderiza a regua com rosa forte para instalacoes de vendas do proprio mes, rosa escuro para vendas de meses anteriores instaladas agora e cinza para vendas do mes ainda nao instaladas. O texto muda de `10 / 8` para leitura operacional como `10 inst | 8 vendas | +2 ant.`, evitando a impressao de erro quando instalacoes superam vendas por efeito de backlog. `DEPLOY_DATE` atualizado para `05/06/2026 18:18`. Validacao: `node --check Code.js` + parse do `<script>` de `Dashboard.html` OK. |
| 05/06/2026 18:03 | deploy manual @877 | **fix(dashboard): barra rosa de instaladas x venda bruta passa a usar o total real de instalacoes de Fibra do mes.** `Dashboard.html` deixa de renderizar `instaladasDaVendaBrutaMes` na barra superior e passa a usar `instalacoesMes`, o mesmo KPI principal de instalacoes por `Data Instalacao`/status `3 - Finalizada/Instalada` para Fibra Alone + Fibra Combo. Isso evita divergencia visual quando a venda bruta e a instalacao pertencem a cohorts/datas diferentes. `DEPLOY_DATE` atualizado para `05/06/2026 18:03`; `clasp push --force` + deploy no deploymentId principal. Validacao: parse do `<script>` de `Dashboard.html` via Node OK. |
